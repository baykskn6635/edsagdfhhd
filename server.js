...
let kanal = "log" // LOG KANAL
let ustKanal = "2.log" // YETKİSİ YETMEYİNCE vb.

client.on("ready", () => {
    client.user.setActivity("Developed By Lowzy");
...
client.on("message", async message => {
    ...
    for (const içindeki of prefikslerim) {
        if (message.content.startsWith(içindeki)) lowzy = içindeki;
    }
    if (!lowzy) return;
    const args = message.content.slice(lowzy.length).trim().split(/ +/g);
